...
/***/ function(module, exports, __webpack_require__) {
	...
	var visualization = new _visualization2.default();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _predict_proba = __webpack_require__(3);
	
	var _predict_proba2 = _interopRequireDefault(_predict_proba);
	
	var _pros_and_cons = __webpack_require__(6);
	
	var _pros_and_cons2 = _interopRequireDefault(_pros_and_cons);
	...
	var Visualization = function () {
	  function Visualization() {
	    ...
	    var dataset_div = _d2.default.select('#datasets_div');
	    dataset_div.selectAll('li').data(['Religion', 'Politeness', 'Sentiment']).enter().append('li').append('a').classed('tablink', true).text(function (d) {
	      return d;
	    }).on('click', function (d) {
	      _d2.default.select('#datasets_div').selectAll('a').classed('active', false);
	      _d2.default.select(this).classed('active', true);
	      self.update_dataset(d);
	    });
	    var button = _d2.default.select('#explain_button_div').append('button');
	  ...
	  _createClass(Visualization, [{
	    ...
	    key: 'update_model_options',
	    value: function update_model_options(side_object) {
	      var self = this;
	      side_object.model_select.html('');
	      side_object.model_select.selectAll('option').data(self.model_options[self.dataset]).enter().append('option').text(function (d) {
	        return d;
	      });
	    ...
	    value: function update_suggestions() {
	      var self = this;
	      var sides = [];
	    ...
	    key: 'update_text',
	    value: function update_text(text) {
	    ...
	    value: function send_request() {
	      ...
	      // %TODO
	      var params = { 'text': text, 'dataset': this.dataset, 'model_left': self.model_left, 'model_right': self.model_right };
	      if (text == '') {
	        return;
	      }
	      this.start_loading(800);
	      params = JSON.stringify(params);
	      _d2.default.xhr("api/explain", "application/json").post(params, function (error, data) {
	    ...
	    value: function update_side(side, text, data) {
	      ...
	      linear_exp['class'] = 1;
	      var explanation = new _pros_and_cons2.default(linear_exp, is_nn);
	    ...
	    value: function build_side(side_object, div_id) {
	      ...
	      var svg = pp_div.append('svg').style('width', pp_width + 'px');
	      side_object.predict_proba = new _predict_proba2.default(svg, ['bla', 'bledfskljflajfd']);
	      var exp_width = Math.min(total_width - pp_width - select_width, 300);
	    ...
	    key: 'start_loading',
	    value: function start_loading(ms) {
	      var self = this;
	      self.is_loading = true;
	      var temp = setTimeout(function () {
	        if (self.is_loading) {
	          _d2.default.select('#loading').style('visibility', 'visible');
	        }
	      }, ms);
	    }
	  }, {
	    ...
	    value: function stop_loading() {
	      ...
	      _d2.default.select('#loading').style('visibility', 'hidden');
	    }
	  }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var PredictProba = function () {
	  ...
	  function PredictProba(svg, class_names) {
	    var _this = this;
	
	    var title = arguments.length <= 2 || arguments[2] === undefined ? 'Prediction probabilities' : arguments[2];
	
	    _classCallCheck(this, PredictProba);
	
	    var width = parseInt(svg.style('width'));
	    this.names = class_names;
	    this.colors_i = _d2.default.scale.category10().domain((0, _lodash.range)(2));
	    this.bar_x = width - 125;
	    var class_names_width = this.bar_x;
	    var bar_width = width - this.bar_x - 32;
	    this.x_scale = _d2.default.scale.linear().range([0, bar_width]);
	    this.bar_height = 17;
	    ...
	    try {
	      for (var _iterator = (0, _lodash.range)(2)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	        var i = _step.value;
	  ...
	  _createClass(PredictProba, [{
	    ...
	    value: function update(predict_probas) {
	      ...
	      var rect = this.bar.selectAll('.colored_rects').data(data);
	      rect.enter().append('rect').classed('colored_rects', true);
	      rect.attr("x", this_object.bar_x).attr("y", function (d, i) {
	        return this_object.bar_y(i);
	      }).attr("height", this_object.bar_height).style("fill", function (d, i) {
	        return this_object.colors_i(i);
	      });
	      rect.transition().duration(1000).attr("width", function (d) {
	        return this_object.x_scale(d);
	      });
	      var prob_text = this.bar.selectAll('.prob_texts').data(data);
	      prob_text.enter().append('text').classed('prob_texts', true);
	      prob_text.attr("fill", "black").attr("y", function (d, i) {
	        return this_object.bar_y(i) + this_object.bar_height - 3;
	      }).style("font", "14px tahoma, sans-serif").text(function (d) {
	        return d.toFixed(2);
	      });
	      prob_text.transition().duration(1000).attr("x", function (d) {
	        return this_object.bar_x + this_object.x_scale(d) + 5;
	      });
	      var text_class_names = this.bar.selectAll('.class_names').data(data);
	      text_class_names.enter().append('text').classed('class_names', true);
	      text_class_names.attr("x", this_object.bar_x - 10).attr("y", function (d, i) {
	        return this_object.bar_y(i) + this_object.bar_height - 3;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _bar_chart = __webpack_require__(7);
	...
	var ProsAndCons = function () {
	  function ProsAndCons(exp) {
	    var positions = arguments.length <= 1 || arguments[1] === undefined ? false : arguments[1];
	
	    _classCallCheck(this, ProsAndCons);
	
	    this.exp = exp;
	    this.class_colors = _d3.default.scale.category10().domain((0, _lodash.range)(2));
	    this.positions = positions;
	  }
	
	  _createClass(ProsAndCons, [{
	    key: 'remove_underline',
	    value: function remove_underline(word_weight) {
	      var ret = [];
	      var _iteratorNormalCompletion = true;
	      var _didIteratorError = false;
	      var _iteratorError = undefined;
	      ...
	      try {
	        for (var _iterator = word_weight[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	          var ww = _step.value;
	
	          var word = ww[0];
	          var idx = word.lastIndexOf('_');
	          if (idx == -1) {
	            ret.push(ww);
	          } else {
	    ...
	    value: function show(div) {
	      ...
	      var colors = [this.class_colors(0), this.class_colors(1)];
	      var word_weight = this.exp['word_weight'];
	      if (this.positions) {
	        word_weight = this.remove_underline(word_weight);
	      }
	      var two_sided = !(0, _lodash.isUndefined)(this.exp['two_sided']);
	
	      if (two_sided) {
	        svg.style('width', '50%');
	        var svg2 = div.append('svg').style('width', '50%');
	        var pos = word_weight.filter(function (x) {
	          return x[1] > 0.00000001;
	        });
	        var neg = word_weight.filter(function (x) {
	          return x[1] < -0.00000001;
	        });
	        var plot1 = new _bar_chart2.default(svg, pos, false, 'Pros', colors);
	        var plot2 = new _bar_chart2.default(svg2, neg, false, 'Cons', colors);
	        var max_height = Math.max(plot1.svg_height, plot2.svg_height);
	        svg.style('height', max_height);
	        svg2.style('height', max_height);
	      } else {
	        var plot = new _bar_chart2.default(svg, word_weight, true, undefined, colors);
	        svg.style('height', plot.svg_height);
	      }
	    }
	  }, {
	    key: 'show_raw_positions',
	    value: function show_raw_positions(div, text) {
	      div.html('');
	      var colors = [this.class_colors(0), this.class_colors(1)];
	      var word_weight = this.exp['word_weight'];
	      var word_lists = [[], []];
	      ...
	      try {
	        for (var _iterator2 = this.exp['word_weight'][Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
	          var _step2$value = _slicedToArray(_step2.value, 2);
	
	          var word = _step2$value[0];
	          ...
	          var word2 = word.substring(0, idx);
	          var start = +word.substring(idx + 1);
	          var end = start + word2.length;
	          if (weight > 0) {
	    ...
	    key: 'show_raw',
	    value: function show_raw(div, text) {
	      if (this.positions) {
	      ...
	      try {
	        for (var _iterator3 = this.exp['word_weight'][Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
	          var _step3$value = _slicedToArray(_step3.value, 2);
	
	          var word = _step3$value[0];
	          var weight = _step3$value[1];
	    ...
	    value: function display_raw_text(div, raw_text) {
	      ...
	      var text_span = div.append('span').text(raw_text);
	      ...
	      try {
	        ...
	        for (var _iterator4 = (0, _lodash.range)(position_lists.length)[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
	          _loop();
	        }
	      } catch (err) {
	    ...
	    value: function wordlists_to_positions(word_lists, raw_text) {
	      ...
	      try {
	        for (var _iterator6 = word_lists[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
	          var words = _step6.value;
	
	          if (words.length === 0) {
	            ret.push([]);
	            continue;
	          }
	          var re = new RegExp("\\b(" + words.join('|') + ")\\b", 'igm');
	          var temp = void 0;
	          var list = [];
...
/***/ function(module, exports, __webpack_require__) {
	...
	function Barchart(svg, exp_array) {
	  ...
	  for (var i = 0; i < exp_array.length; ++i) {
	    ...
	    if (show_numbers) {
	      var bartext = svg.append('text').attr('x', to_the_right ? x_offset + size + 1 : x_offset - size - 1).attr('text-anchor', weight > 0 || !two_sided ? 'begin' : 'end').attr('y', bar_height + yscale(i) + text_height + space_between_bar_and_text).attr('font-size', '10').text(weight.toFixed(2));
	    }
	  }
